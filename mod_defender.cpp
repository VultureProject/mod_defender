...
static int fixer_upper(request_rec *r) {
    ...
    runtimeScanner->rawBody = new string();
    bool eos = false;
    ...
    do {
        ...
        if (rc != APR_SUCCESS) {
            switch (rc) {
                case APR_EOF:
                    ap_log_perror(APLOG_MARK, APLOG_NOTICE, 0, r->pool, "Error reading request body: %s", get_apr_error(r->pool, rc));
                    return -6;
                case APR_TIMEUP:
                    ap_log_perror(APLOG_MARK, APLOG_NOTICE, 0, r->pool, "Error reading request body: %s", get_apr_error(r->pool, rc));
                    return -4;
                case AP_FILTER_ERROR:
                    ...
                    return -3;
                case APR_EGENERAL:
                    ap_log_perror(APLOG_MARK, APLOG_NOTICE, 0, r->pool, "Error reading request body: Client went away.");
                    return -2;
                default:
                    ap_log_perror(APLOG_MARK, APLOG_NOTICE, 0, r->pool, "Error reading request body: %s", get_apr_error(r->pool, rc));
                    return -1;
        ...
        for (apr_bucket *bucket = APR_BRIGADE_FIRST(bb_in);  bucket != APR_BRIGADE_SENTINEL(bb_in); bucket = APR_BUCKET_NEXT(bucket)) {
            const char *buf;
            apr_size_t nbytes;
...
static const char *set_libinjection_sql_flag(cmd_parms *cmd, void *_scfg, int flag) {
    server_config_t *scfg = (server_config_t *) ap_get_module_config(cmd->server->module_config, &defender_module);
...
static const char *set_libinjection_xss_flag(cmd_parms *cmd, void *_scfg, int flag) {
    server_config_t *scfg = (server_config_t *) ap_get_module_config(cmd->server->module_config, &defender_module);
...
static const char *set_learning_flag(cmd_parms *cmd, void *_scfg, int flag) {
    server_config_t *scfg = (server_config_t *) ap_get_module_config(cmd->server->module_config, &defender_module);
    scfg->learning = (bool) flag;
    return NULL;
}

static const char *set_mainrules(cmd_parms *cmd, void *sconf_, const char *arg) {
    if (!strcmp(arg, ";")) {
        return NULL;
    }
...
static const char *set_checkrules(cmd_parms *cmd, void *sconf_, const char *arg1, const char *arg2) {
    return parser->parseCheckRule(cmd->pool, arg1, arg2);
}

static const char *set_basicrules(cmd_parms *cmd, void *sconf_, const char *arg) {
